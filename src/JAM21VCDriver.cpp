...
JAM21VCDriver::JAM21VCDriver(std::shared_ptr<const Config> conf, std::unique_ptr<llvm::Module> mod,
...
View JAM21VCDriver::calcBasicHbView(Event e) const
{
...
View JAM21VCDriver::calcBasicPorfView(Event e) const
{
...
void JAM21VCDriver::calcBasicViews(EventLabel *lab)
{
...
void JAM21VCDriver::calcReadViews(ReadLabel *lab)
{
	const auto &g = getGraph();
	View hb = calcBasicHbView(lab->getPos());
	View porf = calcBasicPorfView(lab->getPos());
...
void JAM21VCDriver::calcWriteViews(WriteLabel *lab)
{
	calcBasicViews(lab);
...
void JAM21VCDriver::calcWriteMsgView(WriteLabel *lab)
{
	const auto &g = getGraph();
	View msg;
...
void JAM21VCDriver::calcRMWWriteMsgView(WriteLabel *lab)
{
	const auto &g = getGraph();
	View msg;
...
void JAM21VCDriver::calcFenceRelRfPoBefore(Event last, View &v)
{
	const auto &g = getGraph();
...
void JAM21VCDriver::calcFenceViews(FenceLabel *lab)
{
	const auto &g = getGraph();
	View hb = calcBasicHbView(lab->getPos());
	View porf = calcBasicPorfView(lab->getPos());
...
void JAM21VCDriver::calcStartViews(ThreadStartLabel *lab)
{
	const auto &g = getGraph();
...
void JAM21VCDriver::calcJoinViews(ThreadJoinLabel *lab)
{
	const auto &g = getGraph();
...
bool JAM21VCDriver::areInDataRace(const MemAccessLabel *aLab, const MemAccessLabel *bLab)
{
...
Event JAM21VCDriver::findRaceForNewLoad(const ReadLabel *rLab)
{
	const auto &g = getGraph();
...
Event JAM21VCDriver::findRaceForNewStore(const WriteLabel *wLab)
{
	const auto &g = getGraph();
...
Event JAM21VCDriver::findDataRaceForMemAccess(const MemAccessLabel *mLab)
{
...
void JAM21VCDriver::changeRf(Event read, Event store)
{
	auto &g = getGraph();
...
void JAM21VCDriver::updateStart(Event create, Event start)
{
	auto &g = getGraph();
...
bool JAM21VCDriver::updateJoin(Event join, Event childLast)
{
	auto &g = getGraph();
...
void JAM21VCDriver::initConsCalculation()
{
	return;
}
