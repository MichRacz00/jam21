 ...
 */
 ...
 JAM21SimpleDriver::JAM21SimpleDriver(std::shared_ptr<const Config> conf, std::unique_ptr<llvm::Module> mod,
				std::unique_ptr<ModuleInfo> MI)
	 : GenMCDriver(conf, std::move(mod), std::move(MI))
 {
	 auto &g = getGraph();
	
	g.addCalculator(std::make_unique<SimpleCalculator>(g),
 ...
 View JAM21SimpleDriver::calcBasicHbView(Event e) const
 {
 ...
 View JAM21SimpleDriver::calcBasicPorfView(Event e) const
 {
 ...
 void JAM21SimpleDriver::calcBasicViews(EventLabel *lab)
 {
 ...
 void JAM21SimpleDriver::calcReadViews(ReadLabel *lab)
 {
	 const auto &g = getGraph();
	 View hb = calcBasicHbView(lab->getPos());
	 View porf = calcBasicPorfView(lab->getPos());
 ...
 void JAM21SimpleDriver::calcWriteViews(WriteLabel *lab)
 {
	 calcBasicViews(lab);
 ...
 void JAM21SimpleDriver::calcWriteMsgView(WriteLabel *lab)
 {
	 const auto &g = getGraph();
	 View msg;
 ...
 void JAM21SimpleDriver::calcRMWWriteMsgView(WriteLabel *lab)
 {
	 const auto &g = getGraph();
	 View msg;
 ...
 void JAM21SimpleDriver::calcFenceRelRfPoBefore(Event last, View &v)
 {
	 const auto &g = getGraph();
 ...
 void JAM21SimpleDriver::calcFenceViews(FenceLabel *lab)
 {
	 const auto &g = getGraph();
	 View hb = calcBasicHbView(lab->getPos());
	 View porf = calcBasicPorfView(lab->getPos());
 ...
 void JAM21SimpleDriver::calcStartViews(ThreadStartLabel *lab)
 {
	 const auto &g = getGraph();
 ...
 void JAM21SimpleDriver::calcJoinViews(ThreadJoinLabel *lab)
 {
	 const auto &g = getGraph();
 ...
 void JAM21SimpleDriver::updateLabelViews(EventLabel *lab, const EventDeps *deps) /* deps ignored */
 {
	 const auto &g = getGraph();
 ...
 bool JAM21SimpleDriver::areInDataRace(const MemAccessLabel *aLab, const MemAccessLabel *bLab)
 {
 ...
 Event JAM21SimpleDriver::findRaceForNewLoad(const ReadLabel *rLab)
 {
	 const auto &g = getGraph();
 ...
 Event JAM21SimpleDriver::findRaceForNewStore(const WriteLabel *wLab)
 {
	 const auto &g = getGraph();
 ...
 Event JAM21SimpleDriver::findDataRaceForMemAccess(const MemAccessLabel *mLab)
 {
 ...
 void JAM21SimpleDriver::changeRf(Event read, Event store)
 {
	 auto &g = getGraph();
 ...
 void JAM21SimpleDriver::updateStart(Event create, Event start)
 {
	 auto &g = getGraph();
 ...
 bool JAM21SimpleDriver::updateJoin(Event join, Event childLast)
 {
	 auto &g = getGraph();
 ...
 void JAM21SimpleDriver::initConsCalculation()
 {
	 return;
 }
