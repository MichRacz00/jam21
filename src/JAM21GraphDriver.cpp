 ...
 */
 ...
 JAM21GraphDriver::JAM21GraphDriver(std::shared_ptr<const Config> conf, std::unique_ptr<llvm::Module> mod,
				std::unique_ptr<ModuleInfo> MI)
	 : GenMCDriver(conf, std::move(mod), std::move(MI))
 {
	 auto &g = getGraph();
	
	g.addCalculator(std::make_unique<JAM21GraphCalculator>(g),
 ...
 View JAM21GraphDriver::calcBasicHbView(Event e) const
 {
 ...
 View JAM21GraphDriver::calcBasicPorfView(Event e) const
 {
 ...
 void JAM21GraphDriver::calcBasicViews(EventLabel *lab)
 {
 ...
 void JAM21GraphDriver::calcReadViews(ReadLabel *lab)
 {
	 const auto &g = getGraph();
	 View hb = calcBasicHbView(lab->getPos());
	 View porf = calcBasicPorfView(lab->getPos());
 ...
 void JAM21GraphDriver::calcWriteViews(WriteLabel *lab)
 {
	 calcBasicViews(lab);
 ...
 void JAM21GraphDriver::calcWriteMsgView(WriteLabel *lab)
 {
	 const auto &g = getGraph();
	 View msg;
 ...
 void JAM21GraphDriver::calcRMWWriteMsgView(WriteLabel *lab)
 {
	 const auto &g = getGraph();
	 View msg;
 ...
 void JAM21GraphDriver::calcFenceRelRfPoBefore(Event last, View &v)
 {
	 const auto &g = getGraph();
 ...
 void JAM21GraphDriver::calcFenceViews(FenceLabel *lab)
 {
	 const auto &g = getGraph();
	 View hb = calcBasicHbView(lab->getPos());
	 View porf = calcBasicPorfView(lab->getPos());
 ...
 void JAM21GraphDriver::calcStartViews(ThreadStartLabel *lab)
 {
	 const auto &g = getGraph();
 ...
 void JAM21GraphDriver::calcJoinViews(ThreadJoinLabel *lab)
 {
	 const auto &g = getGraph();
 ...
 void JAM21GraphDriver::updateLabelViews(EventLabel *lab, const EventDeps *deps) /* deps ignored */
 {
	 const auto &g = getGraph();
 ...
 bool JAM21GraphDriver::areInDataRace(const MemAccessLabel *aLab, const MemAccessLabel *bLab)
 {
 ...
 Event JAM21GraphDriver::findRaceForNewLoad(const ReadLabel *rLab)
 {
	 const auto &g = getGraph();
 ...
 Event JAM21GraphDriver::findRaceForNewStore(const WriteLabel *wLab)
 {
	 const auto &g = getGraph();
 ...
 Event JAM21GraphDriver::findDataRaceForMemAccess(const MemAccessLabel *mLab)
 {
 ...
 void JAM21GraphDriver::changeRf(Event read, Event store)
 {
	 auto &g = getGraph();
 ...
 void JAM21GraphDriver::updateStart(Event create, Event start)
 {
	 auto &g = getGraph();
 ...
 bool JAM21GraphDriver::updateJoin(Event join, Event childLast)
 {
	 auto &g = getGraph();
 ...
 void JAM21GraphDriver::initConsCalculation()
 {
	 return;
 }
