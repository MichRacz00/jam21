...
class VCCalculator : public Calculator {

public:
	VCCalculator(ExecutionGraph &g) : Calculator(g) {}
	...
	std::unique_ptr<Calculator> clone(ExecutionGraph &g) const override {
		return std::make_unique<VCCalculator>(g);
	}

private:
	std::unordered_map<EventLabel*, View> hbClocks;
	std::unordered_map<EventLabel*, int> moClocks;
	...
	bool addMOedge(EventLabel* from, EventLabel* to);
