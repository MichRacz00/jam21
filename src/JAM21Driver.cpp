...
View JAM21Driver::calcBasicHbView(Event e) const
{
...
View JAM21Driver::calcBasicPorfView(Event e) const
{
...
void JAM21Driver::calcBasicViews(EventLabel *lab)
{
...
void JAM21Driver::calcReadViews(ReadLabel *lab)
{
	const auto &g = getGraph();
	View hb = calcBasicHbView(lab->getPos());
	View porf = calcBasicPorfView(lab->getPos());
...
void JAM21Driver::calcWriteViews(WriteLabel *lab)
{
	calcBasicViews(lab);
...
void JAM21Driver::calcWriteMsgView(WriteLabel *lab)
{
	const auto &g = getGraph();
	View msg;
...
void JAM21Driver::calcRMWWriteMsgView(WriteLabel *lab)
{
	const auto &g = getGraph();
	View msg;
...
void JAM21Driver::calcFenceRelRfPoBefore(Event last, View &v)
{
	const auto &g = getGraph();
...
void JAM21Driver::calcStartViews(ThreadStartLabel *lab)
{
	const auto &g = getGraph();
...
void JAM21Driver::calcJoinViews(ThreadJoinLabel *lab)
{
	const auto &g = getGraph();
...
Event JAM21Driver::findRaceForNewLoad(const ReadLabel *rLab)
{
	const auto &g = getGraph();
...
Event JAM21Driver::findRaceForNewStore(const WriteLabel *wLab)
{
	const auto &g = getGraph();
...
Event JAM21Driver::findDataRaceForMemAccess(const MemAccessLabel *mLab)
{
...
void JAM21Driver::changeRf(Event read, Event store)
{
	auto &g = getGraph();
...
void JAM21Driver::updateStart(Event create, Event start)
{
	auto &g = getGraph();
...
bool JAM21Driver::updateJoin(Event join, Event childLast)
{
	auto &g = getGraph();
...
void JAM21Driver::initConsCalculation()
{
	return;
}
